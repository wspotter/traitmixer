...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("persists personality profiles alongside other UI settings", async () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/",
    ...
    saveSettings({
      ...
      personalityProfiles: {
        planner: {
          ...
          mode: "dashboard",
          updatedAt: 123,
          summary: "neutral / balanced / balanced | Global | dashboard",
          metadata: {
            createdAt: 120,
            applyCount: 2,
            lastAppliedAt: 122,
    ...
    expect(loadSettings().personalityProfiles).toMatchObject({
      planner: {
        name: "Planner Mode",
        mode: "dashboard",
        summary: "neutral / balanced / balanced | Global | dashboard",
  ...
  it("normalizes legacy saved personality profiles without metadata", async () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/",
    ...
    const gwUrl = expectedGatewayUrl("");
    const storageKey = `openclaw.control.settings.v1:${gwUrl}`;
    localStorage.setItem(
      storageKey,
      JSON.stringify({
        gatewayUrl: gwUrl,
  ...
  it("drops malformed saved boards and rewrites the cleaned profile map", async () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/",
    ...
    expect(settings.personalityProfiles).toMatchObject({
      good: {
        name: "Good board",
      },
    });
    expect(settings.personalityProfiles).not.toHaveProperty("bad");

    const rewritten = JSON.parse(localStorage.getItem(storageKey) ?? "{}");
    expect(Object.keys(rewritten.personalityProfiles ?? {})).toEqual(["good"]);
  });
