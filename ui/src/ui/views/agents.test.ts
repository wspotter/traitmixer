...
function createProps(overrides: Partial<AgentsProps> = {}): AgentsProps {
  return {
    ...
    onPersonalityTargetChange: () => undefined,
    onPersonalityChannelChange: () => undefined,
    onPersonalityModeChange: () => undefined,
    onPersonalityFieldChange: () => undefined,
...
describe("renderAgents", () => {
  ...
  it("renders the personality tab in the agents panel set", async () => {
    const container = document.createElement("div");
    render(renderAgents(createProps()), container);
    await Promise.resolve();

    const personalityTab = Array.from(
      container.querySelectorAll<HTMLButtonElement>(".agent-tab"),
    ).find((button) => button.textContent?.includes("Personality"));

    expect(personalityTab).toBeDefined();
  });
});
