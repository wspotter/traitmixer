...
export const AGENT_PERSONALITY_DEFAULT_MODE = "default";

export function normalizePersonalityChannel(channel: string | null | undefined): string {
  const normalized = channel?.trim();
  return normalized ? normalized : AGENT_PERSONALITY_ALL_CHANNEL;
}

export function normalizePersonalityMode(mode: string | null | undefined): string {
  const normalized = mode?.trim();
  return normalized ? normalized : AGENT_PERSONALITY_DEFAULT_MODE;
}

export function parsePersonalityGuardrailsText(value: string): string[] {
  return value
    .split(/\r?\n/g)
    .map((entry) => entry.trim())
    .filter(Boolean);
...
export function buildPersonalityScopePath(channel: string, mode: string): string[] {
  const normalizedChannel = normalizePersonalityChannel(channel);
  const normalizedMode = normalizePersonalityMode(mode);
  const path: string[] = [];
  if (normalizedChannel !== AGENT_PERSONALITY_ALL_CHANNEL) {
    path.push("channels", normalizedChannel);
  }
  if (normalizedMode !== AGENT_PERSONALITY_DEFAULT_MODE) {
    path.push("modes", normalizedMode);
  }
  return path;
}
