...
import type { SavedPersonalityProfile } from "./storage.ts";
import type { PersonalityPreviewMode } from "./views/agents-panels-personality.ts";
import { agentLogoUrl } from "./views/agents-utils.ts";
import {
  normalizePersonalityChannel,
...
function createSavedPersonalityProfileId() {
  const generated = globalThis.crypto?.randomUUID?.();
  if (generated) {
    return `personality-${generated}`;
  }
  return `personality-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}

function summarizeSavedPersonalityProfile(
  personality: PersonalityConfig,
  channel: string,
): string | undefined {
  const channelOverride = channel === "*" ? undefined : personality.channels?.[channel];
  const parts = [
    channelOverride?.tone ?? personality.style?.tone,
    channelOverride?.directness ?? personality.style?.directness,
    channelOverride?.responseStyle,
    personality.authority?.confidence,
...
export function renderApp(state: AppViewState) {
  ...
  const upsertSavedBoard = (profile: SavedPersonalityProfile) => {
    const existing = state.settings.personalityProfiles ?? {};
    state.applySettings({
      ...state.settings,
      personalityProfiles: {
        ...existing,
        ...
        ${state.tab === "agents"
          ? lazyRender(lazyAgents, (m) =>
              m.renderAgents({
                ...
                personality: {
                  target: state.agentPersonalityTarget,
                  channel: state.agentPersonalityChannel,
                  mode: state.agentPersonalityMode,
                  profileId: state.agentPersonalityProfileId,
                  profiles: Object.values(state.settings.personalityProfiles ?? {}),
                },
                onRefresh: async () => {
                  ...
                  if (state.agentsPanel === "personality") {
                    void loadChannels(state, false);
                  }
                ...
                onSelectPanel: (panel) => {
                  ...
                  if (panel === "personality") {
                    if (!state.channelsSnapshot || state.channelsError) {
                      void loadChannels(state, false);
                    }
                  }
                  if (panel === "cron") {
                ...
                onPersonalityTargetChange: (target) => {
                  state.agentPersonalityTarget = target;
                },
                onPersonalityChannelChange: (channel) => {
                  state.agentPersonalityChannel = normalizePersonalityChannel(channel);
                },
                onPersonalityModeChange: (mode) => {
                  state.agentPersonalityMode = normalizePersonalityMode(
                    mode,
                  ) as PersonalityPreviewMode;
                },
                onPersonalityProfileChange: (profileId) => {
                  state.agentPersonalityProfileId = profileId;
                },
                onPersonalityProfileSave: ({ agentId }) => {
                  const config = getCurrentConfigValue();
                  const resolved = resolveAgentConfig(config, agentId);
                  const snapshot =
                    state.agentPersonalityTarget === "defaults"
                      ? resolved.defaults?.personality
                      : resolved.entry?.personality;
                  ...
                  const defaultName =
                    state.agentPersonalityTarget === "defaults"
                      ? "Default voice board"
                      : `${agentId} voice board`;
                  const rawName = window.prompt("Name this saved board", defaultName)?.trim();
                  if (!rawName) {
                    return;
                  }
                  const now = Date.now();
                  const profile: SavedPersonalityProfile = {
                    id: createSavedPersonalityProfileId(),
                    name: rawName,
                    target: state.agentPersonalityTarget,
                    channel: state.agentPersonalityChannel,
                    mode: state.agentPersonalityMode,
                    personality: structuredClone(
                      snapshot,
                    ) as SavedPersonalityProfile["personality"],
                    summary:
                      summarizeSavedPersonalityProfile(
                        snapshot as PersonalityConfig,
                        state.agentPersonalityChannel,
                      ) ?? undefined,
                    metadata: {
                      createdAt: now,
                      applyCount: 0,
                  ...
                  upsertSavedBoard(profile);
                  state.agentPersonalityProfileId = profile.id;
                },
                onPersonalityProfileApply: ({ agentId, profileId }) => {
                  const profile = state.settings.personalityProfiles?.[profileId];
                  if (!profile) {
                    return;
                  }
                  const basePath =
                    profile.target === "defaults"
                      ? ["agents", "defaults", "personality"]
                      : (() => {
                          const index = ensureAgentIndex(agentId);
                          return index >= 0
                            ? (["agents", "list", index, "personality"] as Array<string | number>)
                            : null;
                        })();
                  if (!basePath) {
                    return;
                  }
                  updateConfigFormValue(state, basePath, structuredClone(profile.personality));
                  state.agentPersonalityTarget = profile.target;
                  state.agentPersonalityChannel = normalizePersonalityChannel(profile.channel);
                  state.agentPersonalityMode = normalizePersonalityMode(
                    profile.mode,
                  ) as PersonalityPreviewMode;
                  state.agentPersonalityProfileId = profile.id;
                  ...
                  upsertSavedBoard({
                    ...profile,
                    metadata: {
                      createdAt: profile.metadata?.createdAt ?? profile.updatedAt ?? now,
                      applyCount: (profile.metadata?.applyCount ?? 0) + 1,
                      lastAppliedAt: now,
                      ...(typeof profile.metadata?.lastRenamedAt === "number"
                        ? { lastRenamedAt: profile.metadata.lastRenamedAt }
                        : {}),
                    },
                  });
                },
                onPersonalityProfileDelete: (profileId) => {
                  if (!state.settings.personalityProfiles?.[profileId]) {
                    return;
                  }
                  const nextProfiles = { ...state.settings.personalityProfiles };
                  ...
                  if (state.agentPersonalityProfileId === profileId) {
                    state.agentPersonalityProfileId = "";
                  }
                },
                onPersonalityProfileRename: ({ profileId, name }) => {
                  const profile = state.settings.personalityProfiles?.[profileId];
                  if (!profile) {
                    return;
                  }
                  ...
                  const now = Date.now();
                  const nextProfile: SavedPersonalityProfile = {
                    ...profile,
                    name: nextName,
                    summary:
                      profile.summary ||
                      summarizeSavedPersonalityProfile(profile.personality, profile.channel),
                    metadata: {
                      createdAt: profile.metadata?.createdAt ?? profile.updatedAt ?? now,
                      applyCount: profile.metadata?.applyCount ?? 0,
                      ...(typeof profile.metadata?.lastAppliedAt === "number"
                        ? { lastAppliedAt: profile.metadata.lastAppliedAt }
                        : {}),
                      lastRenamedAt: now,
                    },
                    updatedAt: now,
                  };
                  upsertSavedBoard(nextProfile);
                },
                onPersonalityFieldChange: ({ agentId, target, path, value }) => {
                  const basePath =
                    target === "defaults"
                      ? ["agents", "defaults", "personality"]
                      : (() => {
