...
import { buildCliArgs, buildSystemPrompt, loadPromptRefImages } from "./cli-runner/helpers.js";
...
describe("buildSystemPrompt", () => {
  it("injects compiled personality overlay from agent config", () => {
    const cfg = {
      agents: {
        defaults: {
          personality: {
    ...
    expect(prompt).toContain("## Personality Overlay");
    expect(prompt).toContain("- Communication: tone=warm, directness=balanced.");
    expect(prompt).toContain("### Truth & Integrity Guardrails");
    expect(prompt).toContain("- Truthfulness: strict.");
  });
});
