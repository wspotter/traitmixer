...
describe("buildAgentSystemPrompt", () => {
  ...
  it("renders personality overlay after safety and before project context", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      personalityOverlay: [
        "### Behavior",
        "- Communication: tone=warm, directness=balanced.",
        "",
    ...
    const personalityIndex = prompt.indexOf("## Personality Overlay");
    const projectContextIndex = prompt.indexOf("# Project Context");

    expect(personalityIndex).toBeGreaterThan(safetyIndex);
    expect(personalityIndex).toBeLessThan(projectContextIndex);
